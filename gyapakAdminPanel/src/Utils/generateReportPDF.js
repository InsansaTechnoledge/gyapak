...
import footerImage from "../assets/footer-text.png";
import watermarkImage from "../../public/gyapak-logo.png";
import { generateSlugUrl } from "./slugGenerator";
...
const generateLink = (name, id, isNewTrue) => {
  if (!name || !id)
    return console.log("name required in generateLink function ");
...
export const generateReportPDF = async (reportData) => {
  ...
  try {
    footerImageBase64 = await convertImageToBase64(footerImage);
  } catch (error) {
  ...
  try {
    watermarkImageBase64 = await convertImageToBase64(watermarkImage);
  } catch (error) {
  ...
  if (watermarkImageBase64) {
    const logoWidth = 25; // Logo width in mm (increased)
  ...
  const titleText = "Weekly Events Report";
  const titleWidth = doc.getTextWidth(titleText);
  doc.text(titleText, pageWidth - margin - titleWidth, 15);
  ...
  if (
    ...
    // Combine both urgent and soon events into a single table
    const combinedTableData = [
      ...reportData.upcomingEvents.urgent.map((event, index) => {
        ...
        return [
          ...
          fullUrl, // Store full URL but it won't be rendered as text
        ];
      }),
