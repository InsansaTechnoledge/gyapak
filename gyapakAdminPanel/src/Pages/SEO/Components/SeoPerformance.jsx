...
function normalizeGscRows(payload) {
  ...
  const normalized = rows
    .map((r) => {
      ...
      let date =
        r?.date ||
        (Array.isArray(r?.keys) ? r.keys.find((k) => typeof k === "string" && /^\d{4}-\d{2}-\d{2}$/.test(k)) : null);

      // fallback: if key exists as string
      if (!date && typeof r?.key === "string" && /^\d{4}-\d{2}-\d{2}$/.test(r.key)) date = r.key;
...
const SeoPerformance = ({ url }) => {
  ...
  useEffect(() => {
    const getPerformanceData = async () => {
      ...
      setError("");
      try {
        const response = await axios.get(`${url}/gsc/performance?range=${range}`);
        const normalized = normalizeGscRows(response.data);
        setPerformanceData(normalized);
      } catch (e) {
        console.log(e);
        setError(e?.response?.data?.message || e?.message || "Failed to load performance data.");
        setPerformanceData([]);
      } finally {
        setLoading(false);
      }
  ...
  const insights = useMemo(() => {
    ...
    const avgPos = rows.reduce((s, r) => s + (Number.isFinite(r.position) ? r.position : 0), 0) / rows.length;

    const bestClicksDay = rows.reduce((best, r) => (r.clicks > best.clicks ? r : best), rows[0]);
    const bestImprDay = rows.reduce((best, r) => (r.impressions > best.impressions ? r : best), rows[0]);
    ...
    const avgClicksFirst = firstHalf.reduce((s, r) => s + r.clicks, 0) / firstHalf.length;
    const avgClicksSecond = secondHalf.reduce((s, r) => s + r.clicks, 0) / secondHalf.length;
    const clicksDeltaPct =
      avgClicksFirst > 0 ? ((avgClicksSecond - avgClicksFirst) / avgClicksFirst) * 100 : avgClicksSecond > 0 ? 100 : 0;

    const trendWord =
      clicksDeltaPct > 5 ? "up" : clicksDeltaPct < -5 ? "down" : "roughly stable";
    ...
    return (
      <div className="rounded-xl border border-purple-200 bg-white p-3 shadow-md">
        ...
        <div className="mt-2 space-y-1 text-xs text-gray-700">
          <div>Clicks: <span className="font-semibold">{formatNumber(byKey.clicks)}</span></div>
          <div>Impressions: <span className="font-semibold">{formatNumber(byKey.impressions)}</span></div>
          <div>CTR: <span className="font-semibold">{safePct(byKey.ctr)}</span></div>
          <div>Avg Position: <span className="font-semibold">{to2(byKey.position)}</span></div>
        </div>
      </div>
    );
  };
  ...
  return (
    <div className="mt-10 px-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-8">
        <div>
          <h2 className="text-lg font-semibold text-gray-800">Choose Date Range</h2>
        ...
        <div className="mt-8">
          <div className="rounded-2xl border border-purple-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2">
              ...
              <div className="text-xs text-gray-500">
                Tip: clicks & impressions move together; CTR improves when clicks grow faster than impressions.
              </div>
            </div>
            ...
            <div className="mt-4 h-[340px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={performanceData} margin={{ top: 10, right: 20, left: 10, bottom: 0 }}>
                  ...
                  {/* Left axis: volume */}
                  <Line yAxisId="left" type="monotone" dataKey="clicks" dot={false} strokeWidth={2} />
                  <Line yAxisId="left" type="monotone" dataKey="impressions" dot={false} strokeWidth={2} />

                  {/* Right axis: rate/position */}
                  <Line yAxisId="right" type="monotone" dataKey="ctr" dot={false} strokeWidth={2} />
                  <Line yAxisId="right" type="monotone" dataKey="position" dot={false} strokeWidth={2} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-2xl border border-purple-200 bg-white p-4 shadow-sm">
              ...
              <div className="mt-1 text-2xl font-semibold text-gray-900">{formatCompact(insights.totalClicks)}</div>
            </div>
            <div className="rounded-2xl border border-purple-200 bg-white p-4 shadow-sm">
              <div className="text-xs text-gray-500">Total Impressions</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{formatCompact(insights.totalImpr)}</div>
            </div>
            <div className="rounded-2xl border border-purple-200 bg-white p-4 shadow-sm">
              <div className="text-xs text-gray-500">Average CTR</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{safePct(insights.avgCtr)}</div>
            </div>
            <div className="rounded-2xl border border-purple-200 bg-white p-4 shadow-sm">
              <div className="text-xs text-gray-500">Average Position</div>
              <div className="mt-1 text-2xl font-semibold text-gray-900">{to2(insights.avgPos)}</div>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-purple-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-2 text-sm text-gray-700 leading-relaxed space-y-2">
              <p>
                ...
                <span className="font-semibold">{formatNumber(insights.totalClicks)}</span> clicks from{" "}
                <span className="font-semibold">{formatNumber(insights.totalImpr)}</span> impressions.
                That’s an average CTR of <span className="font-semibold">{safePct(insights.avgCtr)}</span>, with an average
                search position of <span className="font-semibold">{to2(insights.avgPos)}</span>.
              </p>

              <p>
                Your best click day was <span className="font-semibold">{prettyDate(insights.bestClicksDay.date)}</span>{" "}
                with <span className="font-semibold">{formatNumber(insights.bestClicksDay.clicks)}</span> clicks.
                The highest visibility (impressions) was on{" "}
                <span className="font-semibold"> {prettyDate(insights.bestImprDay.date)}</span> with{" "}
                <span className="font-semibold">{formatNumber(insights.bestImprDay.impressions)}</span> impressions.
              </p>

              <p>
                Trend check: compared to the first half of the period, your average clicks are{" "}
                <span className="font-semibold">{insights.trendWord}</span>{" "}
                ({insights.clicksDeltaPct >= 0 ? "+" : ""}
                <span className="font-semibold">{to2(insights.clicksDeltaPct)}%</span>).
              </p>

              <div className="mt-3 rounded-xl bg-purple-50 border border-purple-100 p-3 text-xs text-purple-900">
                Quick tip: If impressions rise but clicks don’t, focus on improving CTR (better titles/meta, matching intent,
                rich snippets). If position is high (worse number), focus on content + internal links + topical coverage.
              </div>
            </div>
          </div>
