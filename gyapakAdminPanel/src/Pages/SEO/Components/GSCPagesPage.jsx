    ...
    const fetchPages = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          `/api/gsc/pages?days=${days}&limit=${limit}`
        );
        ...
        <>
          ...
          <div className="mt-6 rounded-2xl border border-purple-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-2 text-sm text-gray-700 space-y-2">
              <p>
                In the last <span className="font-semibold">{days}</span> days,
                your pages got{" "}
                ...
                impressions, which is an overall CTR of{" "}
                <span className="font-semibold">{pct(stats.avgCtr)}</span>.
              </p>
              <p>
                The top page by clicks is{" "}
                <a
                  className="font-semibold text-purple-700 hover:underline break-all"
                  href={stats.topClick.pageUrl}
                  target="_blank"
                  rel="noreferrer"
                >
              ...
              {stats.ctrOpp?.length > 0 && (
                <div className="rounded-xl bg-purple-50 border border-purple-100 p-3 text-xs text-purple-900">
                  ...
                  <div className="mt-2">
                    Fix: rewrite title/meta, add FAQ schema, and improve snippet
                    clarity for these pages.
                  </div>
                </div>
              )}
            </div>
              ...
              <table className="min-w-[1000px] w-full text-sm">
                <thead>
                  <tr className="text-left text-xs text-gray-500 border-b">
                    ...
                    <th className="py-2 cursor-pointer" onClick={() => {}}>
                      Clicks
                    </th>
