...
const DeleteFAQ = () => {
  ...
  const fetchStateOptions = async () => {
    try {
      const response = await axiosInstance.get(`/api/v1i2/faq/states/enum`);
  ...
  const fetchFaqs = async () => {
    try {
      const response = await axiosInstance.get(`/api/v1i2/faq`);
      if (response.data.status === 200) {
    ...
    try {
      ...
      const updatedData = {
        ...
        categories: editFormData.categories
          ? editFormData.categories.split(",").map((item) => item.trim())
          : [],
        seoTags: editFormData.seoTags
          ? editFormData.seoTags.split(",").map((item) => item.trim())
          : [],
        state: editFormData.state || stateOptions[0], // Ensure state is never empty
      };

      const response = await axiosInstance.put(
        `/api/v1i2/faq/${selectedFaq._id}?time=${totalTime}`,
        updatedData
      );
      if (response.data.status === 200) {
    ...
    if (window.confirm("Are you sure you want to delete this FAQ?")) {
      try {
        ...
        const response = await axiosInstance.delete(
          `/api/v1i2/faq/${faqId}?time=${totalTime}`
        );
        if (response.data.status === 200) {
