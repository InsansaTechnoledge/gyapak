...
import { Plus, BookOpen, RefreshCw, AlertCircle, FileQuestionIcon, CheckCircle, X } from 'lucide-react';
import QuestionCard from './QuestionCard';
import { LoadingSkeleton, apiService, Pagination, StatsSkeleton, ErrorMessage, Toast } from './PaginationComponents';
...
const DailyQuestions = () => {
  ...
  const fetchQuestions = useCallback(async (showLoadingState = true) => {
    ...
    try {
      ...
      const questionsData = response.questions || response.data?.questions || response;
      const totalPagesData = response.totalPages || response.data?.totalPages || Math.ceil((response.totalCount || response.data?.totalCount || questionsData.length) / itemsPerPage);
      const totalCountData = response.totalCount || response.data?.totalCount || questionsData.length;
      
      setQuestions(Array.isArray(questionsData) ? questionsData : []);
      setTotalPages(totalPagesData);
      setTotalCount(totalCountData);
    } catch (err) {
      const errorMessage = err.response?.data?.message || err.message || 'Failed to fetch questions. Please try again.';
  ...
  }, [currentPage, itemsPerPage, debouncedSearchTerm, selectedCategory, selectedDifficulty]);
  ...
  const handleDelete = async (questionId) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to delete question. Please try again.';
  ...
  const handleReuse = async (questionId) => {
    setItemLoading(questionId, true);
    try {
      await apiService.reuseQuestion(questionId);
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to reuse question. Please try again.';
      showToast(errorMessage, 'error');
  ...
  const handleCreateQuestionSubmit = async (questionData) => {
    try {
      if(editingQuestion){
        ...
        await apiService.updateQuestion(editingQuestion._id, questionData);
      }else{
      await apiService.createQuestion(questionData);
      }

      showToast(editingQuestion ? 'Question updated successfully!' : 'Question created successfully!', 'success');
      ...
      const errorMessage = err.response?.data?.message || err.message || `Failed to ${editingQuestion ? 'update' : 'create'} question. Please try again.`;
      showToast(errorMessage, 'error');
  ...
  const handleUpdateQuestion = async (questionId, questionData) => {
    setItemLoading(questionId, true);
    try {
      await apiService.updateQuestion(questionId, questionData);
      ...
      const errorMessage = err.response?.data?.message || err.message || 'Failed to update question. Please try again.';
      showToast(errorMessage, 'error');
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-3xl font-bold text-purple-800">Daily MCQ Questions Manager</h2>
          </div>
          <button
        ...
        <p className="text-gray-600">Create, manage, and organize your multiple choice questions with efficient pagination</p>
      </div>
      ...
      <div className="mb-8">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-500">
            <FileQuestionIcon size={48} className="mx-auto mb-4 text-gray-300" />
