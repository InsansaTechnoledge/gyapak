...
import { deletePdf, fetchpdfs } from "../../Services/CurrentAffairSevice";
...
const CurrentAffairManager = () => {
  ...
  const [dailypdfPage, setDailyPdfPage] = useState(false);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this record?")) {
      try {
        const totalTime = Math.floor(Date.now() - startTime.current);
        await deleteCurrentAffairById(id, totalTime);
        fetchData();
      } catch (error) {
  ...
  const handleUpdate = async (updated) => {
    try {
      ...
      await updateCurrentAffairById(updated._id, payload, totalTime);
  ...
  const handlePageChange = (page) => {
    ...
    document
      .querySelector(".current-affairs-table")
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1">
                Manage and organize daily current affairs records
              </p>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={() => setDailyPdfPage((prev) => !prev)}
                className="flex items-center gap-2 bg-blue-600 text-white hover:bg-blue-700 transition px-4 py-2 rounded-lg font-medium"
              >
                <span>
                  {" "}
              ...
              <div className="current-affairs-table bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                        ...
                        <div className="grid grid-cols-12 gap-4 p-4 items-center">
                          ...
                          <div className="col-span-2 text-center">
                            <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                              {rec.affairs.length}{" "}
                              {rec.affairs.length === 1 ? "affair" : "affairs"}
                            </span>
                          </div>
                          ...
                          <div className="col-span-2 text-center">
                            <div className="flex flex-wrap gap-1 justify-center">
                              {[
                                ...new Set(rec.affairs.map((a) => a.language)),
                              ].map((lang, idx) => (
                                <span
                                  key={idx}
                ...
                <div className="bg-gray-50 border-t border-gray-200">
                  <div className="flex justify-between items-center text-sm text-gray-600 p-4 border-b border-gray-200">
                    ...
                    <span>
                      Total Affairs:{" "}
                      {filteredAndSortedRecords.reduce(
                        (acc, rec) => acc + rec.affairs.length,
                        0
                      )}
                    </span>
                  </div>
        ...
        {dailypdfPage && (
          <PdfDataPage data={pdfData} handleDeletePdf={handleDeletePdf} />
        )}
      </div>
