...
export const createBlog = async (req, res) => {
    try {
      ...
      return new APIError(500, [err.message || 'Failed to create blog post']).send(res);
    }
  };
...
export const getBlogBySlug = async (req, res) => {
  try {
    ...
    return new APIError(500, [err.message || 'Failed to get blog post']).send(res);
  }
};
...
export const updateBlog = async (req, res) => {
  try {
    ...
    return new APIError(400, [err.message || 'Failed to update blog post']).send(res);
  }
};
...
export const deleteBlog = async (req, res) => {
  try {
    ...
    return new APIError(500, [err.message || 'Failed to delete blog post']).send(res);
  }
};
...
export const getFeaturedBlogs = async (req, res) => {
  try {
    const blogs = await Blog.find({ featuredPost: true }).sort({ createdAt: -1 });
    return new APIResponse(200, blogs, 'Fetched featured posts').send(res);
  } catch (err) {
    return new APIError(500, [err.message || 'Failed to fetch featured posts']).send(res);
  }
};
...
export const getRelatedBlogs = async (req, res) => {
    try {
      ...
      const normalizedTags = (currentBlog.tags || []).map(tag => tag.toLowerCase());
