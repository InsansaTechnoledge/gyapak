...
export const getOrganizationById = async (req, res) => {
  try {
    ...
    const organization = await Organization.findById(organizationId)
      ...
      .select(
        "_id name abbreviation description logo category createdAt updatedAt"
      );
...
export const getOrganizationDependencies = async (req, res) => {
  try {
    ...
    const events = await Event.find({ organization_id: organizationId })
      .select(
        "name event_type date_of_notification date_of_commencement createdAt"
      )
      .sort({ createdAt: -1 });
    ...
    if (events.length > 0) {
      dependencyData.warnings.push(
        `${events.length} event(s) will be permanently deleted`
      );
    }
    if (faqs.length > 0) {
      dependencyData.warnings.push(
        `${faqs.length} FAQ(s) will be permanently deleted`
      );
    }
    if (authorities.length > 0) {
      dependencyData.warnings.push(
        `Organization will be removed from ${authorities.length} authority(ies)`
      );
    }
    if (categories.length > 0) {
      dependencyData.warnings.push(
        `Organization will be removed from ${categories.length} category(ies)`
      );
    }
...
export const cascadeDeleteOrganization = async (req, res) => {
  ...
  try {
    ...
    const { organizationId, time } = req.query;
    ...
    deletionSummary.deletedItems.authorityReferences =
      updatedAuthorities.modifiedCount;
    ...
    deletionSummary.deletedItems.categoryReferences =
      updatedCategories.modifiedCount;
    ...
    await userActivity.create({
      ...
      event: {
        ...
        eventStamp: {
          title: `Deleted Organization ${organization.name} (CASCADE)`,
        },
        action: "deleted",
...
export const updateOrganization = async (req, res) => {
  try {
    ...
    const newUserActivity = userActivity({
      ...
      event: {
        eventType: "Organization",
        eventId: updatedOrganization._id,
        action: "updated",
        totalTime: Number(time),
      },
    });
