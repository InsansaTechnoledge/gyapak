...
const MobileNav = ({ isOpen, activeDropdown, toggleDropdown, user = { name: "Alex Johnson", email: "alex@example.com", role: "Premium User" } }) => {
  return (
    <div className={`md:hidden ${isOpen ? 'block' : 'hidden'} bg-white dark:bg-gray-900 shadow-xl rounded-b-lg overflow-hidden border-t border-gray-100 dark:border-gray-800`}>
      ...
      <div className="px-4 py-4 bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-900">
        ...
        <div className="mt-2 flex items-center text-xs">
          ...
          <span className="font-medium text-indigo-600 dark:text-indigo-400">{user.role}</span>
        </div>
      </div>
      ...
      <div className="px-2 pt-3 pb-3 divide-y divide-gray-100 dark:divide-gray-800">
        {navItems.map((item, index) => (
          <div key={index} className="py-1">
            <button
              className="w-full text-left block px-4 py-2.5 rounded-lg text-base font-medium text-gray-700 hover:text-indigo-600 hover:bg-indigo-50 dark:text-gray-200 dark:hover:bg-gray-800 flex justify-between items-center transition-all duration-200"
      ...
      <div className="px-4 py-4 border-t border-gray-200 dark:border-gray-700 space-y-3">
        <div className="flex space-x-2">
          <a href="/login" className="flex-1 flex justify-center items-center px-4 py-2.5 text-base font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 dark:bg-gray-750 dark:text-indigo-400 dark:hover:bg-gray-700 rounded-lg transition-colors duration-200">
            Sign In
          </a>
          <a href="/register" className="flex-1 flex justify-center items-center px-4 py-2.5 text-base font-medium text-white bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 rounded-lg shadow-md transition-all duration-200">
            Register
          </a>
        </div>
        
        <a href="/logout" className="flex items-center justify-center w-full px-4 py-2.5 text-sm font-medium text-red-600 hover:text-white hover:bg-red-600 rounded-lg border border-red-200 hover:border-red-600 dark:text-red-400 dark:border-red-900 dark:hover:text-white dark:hover:bg-red-700 transition-colors duration-200">
